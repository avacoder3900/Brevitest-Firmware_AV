...
// TLS root certificate for Supabase (Let's Encrypt)
// Note: This may need to be updated if Supabase changes their certificate chain
    ...
    _totalResponseTime(0)
{
    ...
    memset(_uploadCache, 0, sizeof(_uploadCache));
}
...
bool SupabaseClient::clearCacheEntry(uint8_t index) {
    ...
    // Clear last entry
    memset(&_uploadCache[_cacheCount], 0, sizeof(CachedUpload));
