...
var bcodeCommands = [{
	...
	num: '19',
	name: 'Set LED Baselines',
	params: [],
	description: 'Set LED baselines for each channel.'
}, {
	num: '98',
	  name: 'Comment',
	  params: ['text'],
...
function compileInstruction(cmd, params) {
	...
	var args = elem.params.filter(function(k) {
		return k !== 'comment';
	});
	if (keys.length !== args.length) {
		throw new Error('Parameter count mismatch, command: ' + cmd + ' should have ' + args.length + ', has ' + keys.length);
	}
	return elem.num + (args.length ? paramString(params, args) : '') + '\t';
}
...
function parseReading(line) {
	...
	if (attr.length == 8) {
		...
		return {
			...
			time: Date(parseInt(attr[1])),
			red: parseInt(attr[2]),
			green: parseInt(attr[3]),
			blue: parseInt(attr[4]),
			clear: parseInt(attr[5])
		};
	}
}
