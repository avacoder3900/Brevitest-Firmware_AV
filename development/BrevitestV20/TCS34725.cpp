...
int TCS34725::takeReading (BrevitestSensorRecord *reading, int sample_tries, int it_delay, int blink, int pinNumber, bool debug)
{
    int tries;
    ...
    int count, iterations, laser_delay, sensor_wait, samples = 0;
    bool ready = false, laser_on;
    uint8_t state;
    unsigned long duration, elapsed;
    ...
    for (count = 0; count < iterations; count++) {
        ...
        duration = millis();
        laser_delay = blink ? it_delay / blink : 0;
        sensor_wait = laser_delay / 20;
        while (!ready && ++tries <= SENSOR_WAIT_MAXIMUM_CYCLES) {
          elapsed = millis() - duration;
          laser_on = elapsed > laser_delay && elapsed < (it_delay - laser_delay);
          if (laser_on) {
          ...
          ready = (state & 0x01) == 1;
          if (!ready) {
              if (sample_tries < 0) Serial.print(laser_on ? "*" : ".");
              delay(sensor_wait);
          }
        }
        digitalWrite(pinNumber, LOW);
    ...
    max_clear = reading->clear ? (100 * (max_clear - min_clear)) / reading->clear : 0;
    max_red = reading->red ? (100 * (max_red - min_red)) / reading->red : 0;
    max_green = reading->green ? (100 * (max_green - min_green)) / reading->green : 0;
    max_blue = reading->blue ? (100 * (max_blue - min_blue)) / reading->blue : 0;
    max_lvalue = lvalue ? (100 * (max_lvalue - min_lvalue)) / lvalue : 0;
