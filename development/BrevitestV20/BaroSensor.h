 ...
 *
 * Modifications for Particle platform by Leo Linbeck III, (C) 2019
 */
...
class BaroSensorClass {
 public:
  ...
  int getPressure(BaroOversampleLevel level = OSR_8192);
  ...
  /* Update both temperature and pressure together. This takes less
     ...
     error */
  bool getTempAndPressure(int *temperature,
                          int *pressure,
                          TempUnit tempScale = CELSIUS,
                          BaroOversampleLevel level = OSR_8192);

  inline bool isOK() { return initialised && err == 0; }
  inline uint8_t getError() { return initialised ? err : ERR_NEEDS_BEGIN; }
...
private:
  ...
  int takeReading(uint8_t trigger_cmd, BaroOversampleLevel oversample_level);
};
