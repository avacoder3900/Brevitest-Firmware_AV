...
void read_one_sensor(char sensor_code, int it, int gain, int samples, int blink) {
        ...
        TCS34725 *sensor;
        int tries, lvalue, it_delay, activePin;

        Particle.process();

        if (sensor_code == 'A') {
            sensor = &tcsAssay;
            activePin = pinAssayLaser;
            // turn_on_assay_laser();
        }
        else {
            sensor = &tcsControl;
            activePin = pinControlLaser;
            // turn_on_control_laser();
        }
        ...
        while (reading->clear == 0 && tries++ < 10) {
            lvalue = sensor->takeReading(reading, samples, it_delay, blink, activePin, true);
        }
...
int read_sensors_with_parameters(int integrationTime, int gain, int samples, int blink) {
        RGB.control(true);
        RGB.color(0, 0, 0);

        turn_off_device_LED();

        read_one_sensor('A', integrationTime, gain, samples, blink);
        read_one_sensor('C', integrationTime, gain, samples, blink);
...
int particle_command(String arg) {
    ...
    switch (cmd) {
        ...
        case 3: // move steps
            ...
            return cumulative_steps;
        case 4: // read assay sensor (integration_time, gain, samples)
            ...
            read_sensors_samples = param1;
            read_sensors_blink = param2;
...
void check_assay_sensor_state(bool ledOn) {
    ...
    do {
        ...
        tcsAssay.takeReading(&sensor_state, 1, SENSOR_DEFAULT_IT_DELAY, 1, pinAssayLaser, false);
